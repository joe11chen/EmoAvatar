...
class AudioJobManager:
    ...
    async def submit(self, text: str, emotion_raw: Any, voice, request_perf_ts: float | None = None) -> dict[str, Any]:
    ...
    async def _worker_loop(self):
        while True:
            ...
            try:
                ...
                file_path = await asyncio.to_thread(self._run_job_sync, job.job_id, job.text, job.emotion, job.voice)
    ...
    def _run_job_sync(self, job_id: str, text: str, emotion_name: str, voice: str) -> str:
        ...
        metrics = tts.synthesize_to_wav(text=text, emotion=emotion, voice=voice, output_path=str(output_path))
        logger.info(
