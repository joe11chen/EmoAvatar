...
run_command(
    f"ffmpeg -y -i ./assets/transitions/{TRNASITION1} "
    f"-vf 'setpts={pts_factor}*PTS' "
    f"-r 25 "
    f"-an ./{FINAL_FRAMES_DIR}/{OUT_VIDEO_1}/frame_%04d.png",
    "加速并提取 PNG 序列"
)
...
run_command(
    ...
    f"-r 25 "
    f"-an ./{FINAL_FRAMES_DIR}/{OUT_VIDEO_2}/frame_%04d.png",
    "加速并提取 PNG 序列"
)

print("\n✨ 全部任务完成！")
print(f"最终图片保存在: {FINAL_FRAMES_DIR}")